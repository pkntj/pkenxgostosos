...
module.exports = {
  name: "lírica ",
  description: "Mostrando a lírica da música pesquisada",
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!lyrics)
      return client.sendTime(
        message.channel,
        `**Nenhuma lírica foi encontrada para -** \`${SongTitle}\``
      );
    lyrics = lyrics.split("\n"); //spliting into lines
    ...
    let Pages = SplitedLyrics.map((ly) => {
      let em = new MessageEmbed()
        .setAuthor(`Lírica para: ${SongTitle}`, client.botconfig.IconURL)
    ...
    options: [
      {
        ...
        value: "Música",
        type: 3,
        description: "Escreva o nome da música para pesquisar",
        required: false,
      },
    ],
    /**
